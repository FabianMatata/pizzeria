...
function finalTally() {
    function total(totalPrice) {
        ...
        totalT.innerHTML = "Total to be paid: <span class='dollar'>KSh " + totalPrice + "<span>";
    }
    ...
    for (var s = 0; s < paraContent.length; s++) {
        paraSplit[s] = paraContent[s].split(' KSh');
    }
    var totalPrice = 0;
    for (var p = 0; p < paras.length - 1; p++) {
        paras[p].innerHTML = paraSplit[p][0] + " <span class='dollar'>KSh " + paraSplit[p][1] + "</span>";
        var prices = [];
        prices.push(paraSplit[p][1]);
        ...
        if (check() == 0) {
            ...
            for (var c = 0; c < inputs.length; c++) {
                ...
                if (inputs[c].checked && inputs[c].hasAttribute('data-price')) {
                    ...
                    paras[paras.length - 1].innerHTML = ingredients + ' <span>KSh ' + topay + '</span>';
                }
                //otherwise if it is checked just get the text and output it into the receipt. 
                    ...
                    paras[paras.length - 1].innerHTML = ingredients + ' <span>KSh 0</span>';
                }
            }
            nextS();
        } else if (check() > 1) {
            for (var j = 0; j < CInputs().length; j++) {
                ...
                if (CInputs()[j] === CInputs()[0] && CInputs()[j].hasAttribute('data-price') === false) {
                    ...
                    paras[paras.length - 1].innerHTML = ingredients + ' <span> KSh 1</span>';
